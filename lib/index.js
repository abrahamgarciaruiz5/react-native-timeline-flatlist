...
export default class Timeline extends PureComponent {
  ...
  extraData={this.state.rowHeights}
...
this.state = {
  ...this.state,
  rowHeights: {}, // { filaIndex: alturaMaxima }
};
      ...
      case "single-column-left":
        content = (
          <View style={[styles.rowContainer, this.props.rowContainerStyle]}>
           ...
           {this.renderEvent(item, index, filaIndex)}
...
_renderEvent(rowData, rowID, filaIndex) {
      ...
      <View
        style={[
          ...
          rowData.eventContainerStyle,
            // Aplica la altura máxima de la fila si existe
        this.props.columnFormat === "two-column" && this.state.rowHeights[filaIndex]
          ? { minHeight: this.state.rowHeights[filaIndex] }
          : {},
        ]}
        onLayout={(evt) => {
          if (this.props.columnFormat === "two-column") {
          const { height } = evt.nativeEvent.layout;
          this.setState((prev) => {
            const prevHeight = prev.rowHeights[filaIndex] || 0;
        ...
        } else if (!this.state.x && !this.state.width) {
